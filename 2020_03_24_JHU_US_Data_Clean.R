...
jhu_state_confirmed_cases %>% write_rds(paste0(date,"jhu_state_confirmed_cases.rds"))
jhu_state_confirmed_cases <- read_rds(paste0(date,"jhu_state_confirmed_cases.rds"))
...
jhu_state_confirmed_deaths %>% write_rds(paste0(date,"jhu_state_deaths.rds"))
jhu_state_confirmed_deaths <- read_rds(paste0(date,"jhu_state_deaths.rds"))
...
jhu_state_combined <- read_rds(paste0(date,"jhu_state_combined.rds"))
...
jhu_global_combined <- read_rds(paste0(date,"jhu_global_combined.rds"))
...
nyt_us_states <- read_csv("https://raw.githubusercontent.com/nytimes/covid-19-data/master/us-states.csv") %>%
  clean_names() %>%
  rename(case_total=cases,
         death_total=deaths,
         state_fullname=state,
         date_fmt=date) %>%
  left_join(jhu_confirmed_cases_state_level %>%
              distinct(state_fullname,state_abb,
                       lat,long),
            by="state_fullname") %>%
  left_join(state_name_df %>%
              distinct(state_abb) %>%
...
nyt_us_overall <- nyt_us_states %>%
  group_by(date_fmt) %>%
  summarise(case_total=sum(case_total),
            death_total=sum(death_total),
            case_total_daily=sum(case_total_daily),
            death_total_daily=sum(death_total_daily)) %>%
  ungroup()
...
covid_tracking_states_w_pop <- read_rds(paste0(date,"CTP_Data.rds"))
